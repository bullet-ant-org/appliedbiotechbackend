...
exports.modifyTrackingOrderStatus = async (req, res, next) => {
  try {
    ...
    const updatedModelRecord = await Order.findByIdAndUpdate(req.params.id, { status }, { new: true });
    
    await sendEmail(
      updatedModelRecord.email, 
      `Tracking Update Notice: ${updatedModelRecord.trackingCode}`, 
      `<p>Your order state transformation progress indicator has transitioned to: <strong>${status.toUpperCase()}</strong></p>`
    );
